...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${allProducts[i].name} has ${allProducts[i]} votes in ${allProducts[i].views} veiws`);
    }
  }
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = `${allProducts[i].name} has ${allProducts[i]} votes in ${allProducts[i].views} veiws`;
